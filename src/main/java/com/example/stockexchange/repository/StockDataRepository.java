...
@Repository
public interface StockDataRepository extends R2dbcRepository<StockData, Integer> {
    ...
    Flux<StockData> priceDifferenceBetweenShares();

//    @Query("SELECT * FROM stock_data WHERE symbol in (:symbols)")
//    Flux<StockData> findBySymbol(@Param("symbols") Flux<String> symbols);

        @Query("SELECT * FROM stock_data WHERE symbol in (:symbol)")
        Flux<StockData> findBySymbol(@Param("symbol") List<String> symbol);
}
